...
from src.schemas.entity import (UserRegister, UserInDB, UserLogin, TokenResponse, LogoutRequest, GetUserInfoRequest,
                                UpdateUserCredentialsRequest, UserResponse, UserLoginHistoryResponse)
...
@router.post(
    '/refresh',
    response_model=TokenResponse,
    summary="Refresh a token for user"
)
...
async def update_user_credentials(
        update_request: UpdateUserCredentialsRequest = Body(..., description="User credentials update data"),
        user_service: UserService = Depends(get_user_service),
        access_token: str = Depends(OAuth2PasswordBearer(tokenUrl="token")),
) -> User:
    """
    ...
    try:
        ...
        updated_user = await user_service.update_user_credentials(user_id, update_request)
...
@router.get("/login-history", response_model=PaginatedResponse[UserLoginHistoryResponse])
async def get_user_login_history(
    ...
    try:
        ...
        login_history, total_count = await user_service.get_login_history_paginated(user_id, pagination)
...
async def auth_callback(provider: str, request: Request, user_service: UserService = Depends(get_user_service)):
    # Dynamically get the correct OAuth provider based on the URL parameter
    oauth_provider = getattr(oauth, provider, None)
    if not oauth_provider:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Unsupported OAuth provider.")
    ...
    return JSONResponse(content={"access_token": token_response, "token_type": "bearer"})
