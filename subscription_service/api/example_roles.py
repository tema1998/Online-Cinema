...
from src.schemas.entity import RoleInDB, RoleCreate, RolesInDB, UpdateRoleRequest, UserPermissions, \
...
async def assign_role_to_user(
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"detail": f"Role '{role_data.role_name}' successfully assigned to user with ID: '{user_id}'."}
    )
...
async def remove_role_from_user(
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"detail": f"Role '{role_data.role_name}' successfully removed from user with ID: '{user_id}'."}
    )
