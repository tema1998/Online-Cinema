...
class TestFilmsAPI:
    ...
    def replace_key(cls, data, old_key, new_key):
        if isinstance(data, dict):
            return {
                new_key if key == old_key else key: cls.replace_key(
    ...
    async def test_film_by_id(self, make_get_request):
        ...
        response_body_updated = TestFilmsAPI.replace_key(
            response.body, "full_name", "name"
        )
        updated_film_sorted = TestFilmsAPI.sort_nested_dict(updated_film)
        response_body_updated_sorted = TestFilmsAPI.sort_nested_dict(
            response_body_updated
        )
    ...
    async def test_sort_films(self, make_get_request):
        ...
        sorted_film_data_desc = sorted(
            transformed_data, key=lambda x: x["imdb_rating"], reverse=True
        )
        sorted_film_data_asc = sorted(
            transformed_data, key=lambda x: x["imdb_rating"], reverse=False
        )
    ...
    async def test_sort_films_and_filter(self, make_get_request):
        ...
        response_desc = await make_get_request(
            f"{api_url}", {"sort": "-imdb_rating", "genre": genre_id}
        )
    ...
    async def test_films_pagination(self, make_get_request):
        ...
        response = await make_get_request(
            f"{api_url}", {"page_size": PAGE_SIZE, "page": pages_number}
        )
