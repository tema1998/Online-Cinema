...
indices = [
    {"index_name": "movies", "index_settings": movie_index, "data": film_data},
...
async def create_indices(es_remove_if_exists_and_create_index, es_remove_index):
    ...
    for index in indices:
        await es_remove_if_exists_and_create_index(
            index_name=index["index_name"], index_settings=index["index_settings"]
        )
    yield None
    for index in indices:
...
async def tests_setup(create_indices, es_write_data):
    ...
    """
    for index in indices:
        bulk_query = create_bulk_query(
            index_name=index["index_name"], data=index["data"]
        )
        await es_write_data(
            index_name=index["index_name"], data=bulk_query, refresh="wait_for"
        )
