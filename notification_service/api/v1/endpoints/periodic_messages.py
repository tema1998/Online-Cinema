...
from notification_service.api.v1.dependencies import get_periodic_task_service
...
from notification_service.schemas.messages import (
    ...
    PeriodicTaskIdRequest,
    PeriodicTaskParamsRequest,
)
from notification_service.services.periodic_messages import (
...
async def pause_periodic_task(
    ...
    try:
        result = await periodic_message_service.pause_task(
            task_id=periodic_task_params.task_id
        )
...
async def create_periodic_message(
    ...
    try:
        ...
        periodic_message_service = PeriodicMessageService(
            broker_url=settings.rabbitmq_connection_url
        )
        ...
        result = scheduler.add_job(
            ...
            kwargs={
                ...
                "message": message,
                "message_transfer": "email",
                "queue_name": settings.scheduled_message_queue,
            },
            trigger="interval",
            seconds=periodic_task_params.interval_in_seconds,
            name=periodic_task_params.name,
        )
