...
class Settings(BaseProjectSettings):
    # Middleware settings
    middleware_secret_key: str = Field(
        "practix", alias="NOTIFICATION_MIDDLEWARE_SECRET_KEY"
    )
    ...
    confirmation_base_url: str = Field(
        "http://localhost:8000/confirm-email", alias="CONFIRMATION_BASE_URL"
    )
    ...
    get_user_info_url: str = Field(
        "http://auth_service:8081/api/v1/auth/get-user-info", alias="GET_USER_INFO_URL"
    )
